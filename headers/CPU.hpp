...
//     is_reading_time = 1;
  ...
  void emulate_pmaxsb(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src = instr->operands[1];

      if (dst.size != 128) {
          LOG(L"[!] Unsupported operand size for PMAXSB: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m128i>(dst, 128, a_val)) {
          LOG(L"[!] Failed to read first operand (dst) for PMAXSB");
          return;
      }
      if (!read_operand_value<__m128i>(src, 128, b_val)) {
          LOG(L"[!] Failed to read second operand (src) for PMAXSB");
          return;
      }

      alignas(16) int8_t a_arr[16], b_arr[16], out[16];
      _mm_store_si128((__m128i*)a_arr, a_val);
      _mm_store_si128((__m128i*)b_arr, b_val);
      ...
      if (!write_operand_value<__m128i>(dst, 128, result)) {
          LOG(L"[!] Failed to write result for PMAXSB");
          return;
      }

      LOG(L"[+] PMAXSB executed (128-bit)");
  }
  void emulate_pmaxsw(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src = instr->operands[1];

      if (dst.size != 128) {
          LOG(L"[!] Unsupported operand size for PMAXSW: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m128i>(dst, 128, a_val)) {
          LOG(L"[!] Failed to read first operand (dst) for PMAXSW");
          return;
      }
      if (!read_operand_value<__m128i>(src, 128, b_val)) {
          LOG(L"[!] Failed to read second operand (src) for PMAXSW");
          return;
      }

      alignas(16) int16_t a_arr[8], b_arr[8], out[8];
      _mm_store_si128((__m128i*)a_arr, a_val);
      _mm_store_si128((__m128i*)b_arr, b_val);
      ...
      if (!write_operand_value<__m128i>(dst, 128, result)) {
          LOG(L"[!] Failed to write result for PMAXSW");
          return;
      }

      LOG(L"[+] PMAXSW executed (128-bit)");
  }
  void emulate_pmaxsd(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src = instr->operands[1];

      if (dst.size != 128) {
          LOG(L"[!] Unsupported operand size for PMAXSD: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m128i>(dst, 128, a_val)) {
          LOG(L"[!] Failed to read first operand (dst) for PMAXSD");
          return;
      }
      if (!read_operand_value<__m128i>(src, 128, b_val)) {
          LOG(L"[!] Failed to read second operand (src) for PMAXSD");
          return;
      }
      ...
      if (!write_operand_value<__m128i>(dst, 128, result)) {
          LOG(L"[!] Failed to write result for PMAXSD");
          return;
      }

      LOG(L"[+] PMAXSD executed (128-bit)");
  }
  void emulate_vpmaxsb(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src1 = instr->operands[1];
      const auto& src2 = instr->operands[2];

      if (dst.size != 256) {
          LOG(L"[!] Unsupported operand size for VPMAXSB: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m256i>(src1, 256, a_val)) {
          LOG(L"[!] Failed to read src1 for VPMAXSB");
          return;
      }
      if (!read_operand_value<__m256i>(src2, 256, b_val)) {
          LOG(L"[!] Failed to read src2 for VPMAXSB");
          return;
      }

      alignas(32) int8_t a_arr[32], b_arr[32], out[32];
      _mm256_store_si256((__m256i*)a_arr, a_val);
      _mm256_store_si256((__m256i*)b_arr, b_val);
      ...
      if (!write_operand_value(dst, 256, result)) {
          LOG(L"[!] Failed to write result for VPMAXSB");
          return;
      }

      LOG(L"[+] VPMAXSB executed (256-bit)");
  }
  void emulate_vpmaxsw(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src1 = instr->operands[1];
      const auto& src2 = instr->operands[2];

      if (dst.size != 256) {
          LOG(L"[!] Unsupported operand size for VPMAXSW: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m256i>(src1, 256, a_val)) {
          LOG(L"[!] Failed to read src1 for VPMAXSW");
          return;
      }
      if (!read_operand_value<__m256i>(src2, 256, b_val)) {
          LOG(L"[!] Failed to read src2 for VPMAXSW");
          return;
      }

      alignas(32) int16_t a_arr[16], b_arr[16], out[16];
      _mm256_store_si256((__m256i*)a_arr, a_val);
      _mm256_store_si256((__m256i*)b_arr, b_val);
      ...
      if (!write_operand_value(dst, 256, result)) {
          LOG(L"[!] Failed to write result for VPMAXSW");
          return;
      }

      LOG(L"[+] VPMAXSW executed (256-bit)");
  }
  void emulate_vpmaxsd(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src1 = instr->operands[1];
      const auto& src2 = instr->operands[2];

      if (dst.size != 256) {
          LOG(L"[!] Unsupported operand size for VPMAXSD: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m256i>(src1, 256, a_val)) {
          LOG(L"[!] Failed to read src1 for VPMAXSD");
          return;
      }
      if (!read_operand_value<__m256i>(src2, 256, b_val)) {
          LOG(L"[!] Failed to read src2 for VPMAXSD");
          return;
      }
      ...
      if (!write_operand_value(dst, 256, result)) {
          LOG(L"[!] Failed to write result for VPMAXSD");
          return;
      }
  ...
  void emulate_pminsb(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src = instr->operands[1];

      if (dst.size != 128) {
          LOG(L"[!] Unsupported operand size for PMINSB: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m128i>(dst, 128, a_val)) {
          LOG(L"[!] Failed to read first operand (dst) for PMINSB");
          return;
      }
      if (!read_operand_value<__m128i>(src, 128, b_val)) {
          LOG(L"[!] Failed to read second operand (src) for PMINSB");
          return;
      }

      __m128i result = _mm_min_epi8(a_val, b_val);

      if (!write_operand_value<__m128i>(dst, 128, result)) {
          LOG(L"[!] Failed to write result for PMINSB");
          return;
      }

      LOG(L"[+] PMINSB executed (128-bit)");
  }
  void emulate_vpminsb(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src1 = instr->operands[1];
      const auto& src2 = instr->operands[2];

      if (dst.size != 256) {
          LOG(L"[!] Unsupported operand size for VPMINSB: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m256i>(src1, 256, a_val)) {
          LOG(L"[!] Failed to read src1 for VPMINSB");
          return;
      }
      if (!read_operand_value<__m256i>(src2, 256, b_val)) {
          LOG(L"[!] Failed to read src2 for VPMINSB");
          return;
      }

      __m256i result = _mm256_min_epi8(a_val, b_val);

      if (!write_operand_value<__m256i>(dst, 256, result)) {
          LOG(L"[!] Failed to write result for VPMINSB");
          return;
      }
  ...
  void emulate_pminsw(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src = instr->operands[1];

      if (dst.size != 128) {
          LOG(L"[!] Unsupported operand size for PMINSW: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m128i>(dst, 128, a_val)) {
          LOG(L"[!] Failed to read first operand (dst) for PMINSW");
          return;
      }
      if (!read_operand_value<__m128i>(src, 128, b_val)) {
          LOG(L"[!] Failed to read second operand (src) for PMINSW");
          return;
      }
      __m128i result = _mm_min_epi16(a_val, b_val);

      if (!write_operand_value<__m128i>(dst, 128, result)) {
          LOG(L"[!] Failed to write result for PMINSW");
          return;
      }

      LOG(L"[+] PMINSW executed (128-bit)");
  }
  void emulate_vpminsw(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& src1 = instr->operands[1];
      const auto& src2 = instr->operands[2];

      if (dst.size != 256) {
          LOG(L"[!] Unsupported operand size for VPMINSW: " << dst.size);
          return;
      }
      ...
      if (!read_operand_value<__m256i>(src1, 256, a_val)) {
          LOG(L"[!] Failed to read src1 for VPMINSW");
          return;
      }
      if (!read_operand_value<__m256i>(src2, 256, b_val)) {
          LOG(L"[!] Failed to read src2 for VPMINSW");
          return;
      }

      __m256i result = _mm256_min_epi16(a_val, b_val);

      if (!write_operand_value<__m256i>(dst, 256, result)) {
          LOG(L"[!] Failed to write result for VPMINSW");
          return;
      }
  ...
  void emulate_vgatherdpd(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& mem_op = instr->operands[1];
      const auto& mask_op = instr->operands[2];
      ...
      if (width == 128) {
          ...
          __m128d mask_pd = get_register_value<__m128d>(mask_op.reg.value);
          __m128i mask_i = _mm_castpd_si128(mask_pd);
          alignas(16) uint64_t mask_arr[2];
          ...
          alignas(16) double result_arr[2];
          __m128d prev_dst = get_register_value<__m128d>(dst.reg.value);
          _mm_storeu_pd(result_arr, prev_dst);
          ...
          for (int i = 0; i < 2; ++i) {
              ...
              if (mem_op.mem.base != ZYDIS_REGISTER_NONE) {
                  base_val = get_register_value<uint64_t>(mem_op.mem.base);
                  if (mem_op.mem.base == ZYDIS_REGISTER_RIP) base_val += instr->info.length;
              }

              uint64_t addr = base_val + static_cast<uint64_t>(indices[i]) * mem_op.mem.scale + mem_op.mem.disp.value;
              if (!ReadMemory(addr, &result_arr[i], sizeof(double))) {
                  LOG(L"[!] Failed to read memory lane " << i << " in VGATHERDPD(128)");
                  result_arr[i] = 0.0;
              }
          }

          __m128d result = _mm_loadu_pd(result_arr);
          write_operand_value(dst, 128, result);
          LOG(L"[+] VGATHERDPD executed (128-bit)");
      }
      else if (width == 256) {
          ...
          __m256d mask_pd256 = get_register_value<__m256d>(mask_op.reg.value);
          __m256i mask_i256 = _mm256_castpd_si256(mask_pd256);
          alignas(32) uint64_t mask_arr[4];
          _mm256_storeu_si256((__m256i*)mask_arr, mask_i256);

          __m256i zero_mask = _mm256_setzero_si256();
          write_operand_value(mask_op, 256, zero_mask);


          alignas(32) double result_arr[4];
          __m256d prev_dst = get_register_value<__m256d>(dst.reg.value);
          _mm256_storeu_pd(result_arr, prev_dst);
          ...
          for (int i = 0; i < 4; ++i) {
              ...
              uint64_t addr = base_val + static_cast<uint64_t>(indices[i]) * mem_op.mem.scale + mem_op.mem.disp.value;
              if (!ReadMemory(addr, &result_arr[i], sizeof(double))) {
                  LOG(L"[!] Failed to read memory lane " << i << " in VGATHERDPD(256)");
                  result_arr[i] = 0.0;
              }
          }

          __m256d result = _mm256_loadu_pd(result_arr);
          write_operand_value(dst, 256, result);
          LOG(L"[+] VGATHERDPD executed (256-bit)");
      }
      else {
          LOG(L"[!] Unsupported width in VGATHERDPD: " << width);
      }
  }
  void emulate_vgatherqpd(const ZydisDisassembledInstruction* instr) {
      const auto& dst = instr->operands[0];
      const auto& mem_op = instr->operands[1];
      const auto& mask_op = instr->operands[2];
      ...
      if (width == 128) {
          // indices: 2 x i64 stored in a __m128i
          __m128i index_vec = get_register_value<__m128i>(mem_op.mem.index);
          alignas(16) int64_t indices[2];
          _mm_storeu_si128((__m128i*)indices, index_vec);

          // mask: treat __m128d as integer bits (2 x 64-bit lanes)
          __m128d mask_pd = get_register_value<__m128d>(mask_op.reg.value);
          __m128i mask_i = _mm_castpd_si128(mask_pd);
          alignas(16) uint64_t mask_arr[2];
          _mm_storeu_si128((__m128i*)mask_arr, mask_i);

          // clear mask register (emulate behavior in your other functions)
          __m128i zero_mask = _mm_setzero_si128();
          write_operand_value(mask_op, 128, zero_mask);

          // start from previous dst to preserve lanes where mask is 0
          alignas(16) double result_arr[2];
          __m128d prev_dst = get_register_value<__m128d>(dst.reg.value);
          _mm_storeu_pd(result_arr, prev_dst);

          for (int i = 0; i < 2; ++i) {
              // check MSB of 64-bit mask lane
              if (!(mask_arr[i] & 0x8000000000000000ULL)) continue;
              ...
              uint64_t addr = base_val + static_cast<uint64_t>(indices[i]) * mem_op.mem.scale + mem_op.mem.disp.value;
              if (!ReadMemory(addr, &result_arr[i], sizeof(double))) {
                  LOG(L"[!] Failed to read memory lane " << i << " in VGATHERQPD(128)");
                  result_arr[i] = 0.0;
              }
          }
          ...
          __m128d result = _mm_loadu_pd(result_arr);
          write_operand_value(dst, 128, result);
          LOG(L"[+] VGATHERQPD executed (128-bit)");
      }
      else if (width == 256) {
          // indices: 4 x i64 stored in a __m256i
          __m256i index_vec = get_register_value<__m256i>(mem_op.mem.index);
          alignas(32) int64_t indices[4];
          _mm256_storeu_si256((__m256i*)indices, index_vec);

          // mask: treat __m256d as integer bits (4 x 64-bit lanes)
          __m256d mask_pd256 = get_register_value<__m256d>(mask_op.reg.value);
          __m256i mask_i256 = _mm256_castpd_si256(mask_pd256);
          alignas(32) uint64_t mask_arr[4];
          _mm256_storeu_si256((__m256i*)mask_arr, mask_i256);
          ...
          for (int i = 0; i < 4; ++i) {
              ...
              uint64_t addr = base_val + static_cast<uint64_t>(indices[i]) * mem_op.mem.scale + mem_op.mem.disp.value;
              if (!ReadMemory(addr, &result_arr[i], sizeof(double))) {
                  LOG(L"[!] Failed to read memory lane " << i << " in VGATHERQPD(256)");
                  result_arr[i] = 0.0;
              }
          }
          ...
          __m256d result = _mm256_loadu_pd(result_arr);
          write_operand_value(dst, 256, result);
          LOG(L"[+] VGATHERQPD executed (256-bit)");
      }
      else {

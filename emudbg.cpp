...
int wmain(int argc, wchar_t *argv[]) {
  if (argc < 2) {
    wprintf(L"Usage: %s <exe_path> [-m target.dll] [-r <hex_rva>] [-b "
            L"software|hardware|noexec]\n",
            argv[0]);
    wprintf(L"  Example:\n");
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == L"-p" && i + 1 < argc) {
      ...
      if (type == L"hardware")
        bpType = BreakpointType::Hardware;
      else if (type == L"software")
        bpType = BreakpointType::Software;
      else if (type == L"noexec")
        bpType = BreakpointType::ExecGuard;
      else {
  ...
  if (pfnGetEnabledXStateFeatures == NULL) {
    ...
    pfnGetEnabledXStateFeatures = (PGETENABLEDXSTATEFEATURES)GetProcAddress(
        hm, "GetEnabledXStateFeatures");
    pfnInitializeContext =
        (PINITIALIZECONTEXT)GetProcAddress(hm, "InitializeContext");
    pfnGetXStateFeaturesMask =
        (PGETXSTATEFEATURESMASK)GetProcAddress(hm, "GetXStateFeaturesMask");
    pfnLocateXStateFeature =
        (LOCATEXSTATEFEATURE)GetProcAddress(hm, "LocateXStateFeature");
    pfnSetXStateFeaturesMask =
        (SETXSTATEFEATURESMASK)GetProcAddress(hm, "SetXStateFeaturesMask");

    if (pfnGetEnabledXStateFeatures == NULL || pfnInitializeContext == NULL ||
        pfnGetXStateFeaturesMask == NULL || pfnLocateXStateFeature == NULL ||
  ...
  while (true) {
    ...
    case LOAD_DLL_DEBUG_EVENT: {
      ...
      if (ld.lpImageName && ld.fUnicode) {
        ...
        if (ReadProcessMemory(pi.hProcess, (LPCVOID)ld.lpImageName, &ptr,
                              sizeof(ptr), nullptr) &&
            ptr &&
            ReadProcessMemory(pi.hProcess, (LPCVOID)ptr, buffer,
                              sizeof(buffer) - sizeof(wchar_t), nullptr)) {

          std::wstring loadedName(buffer);
          std::wstring lowerLoaded = loadedName;
          std::transform(lowerLoaded.begin(), lowerLoaded.end(),
                         lowerLoaded.begin(), ::towlower);
          ...
          std::transform(lowerPatchTarget.begin(), lowerPatchTarget.end(),
                         lowerPatchTarget.begin(), ::towlower);
          if ((lowerLoaded.find(lowerPatchTarget) != std::wstring::npos) &&
              patchSectionAddress == 0 && !patchSection.empty()) {
            uint64_t dllBase = reinterpret_cast<uint64_t>(ld.lpBaseOfDll);
            patchModule_File_Path = lowerLoaded;

            IMAGE_DOS_HEADER dosHdr{};
            ReadProcessMemory(pi.hProcess, ld.lpBaseOfDll, &dosHdr,
                              sizeof(dosHdr), nullptr);

            IMAGE_NT_HEADERS64 ntHdr{};
            ReadProcessMemory(
                pi.hProcess,
                (LPCVOID)((uint64_t)ld.lpBaseOfDll + dosHdr.e_lfanew), &ntHdr,
                sizeof(ntHdr), nullptr);
            ...
            for (DWORD i = 0; i < numberOfSections; i++) {
              ...
              ReadProcessMemory(pi.hProcess,
                                (LPCVOID)((uint64_t)ld.lpBaseOfDll +
                                          sectionOffset + i * sizeof(secHdr)),
                                &secHdr, sizeof(secHdr), nullptr);

              size_t cmpLen =
                  min(patchSections.size(), (size_t)IMAGE_SIZEOF_SHORT_NAME);
              sections.push_back(secHdr);
              if (strncmp((char *)secHdr.Name, patchSections.c_str(), cmpLen) ==
                  0) {
                patch_modules_ranges.first = dllBase;
                patch_modules_ranges.second =
                    dllBase + ntHdr.OptionalHeader.SizeOfImage;

                patchSectionAddress =
                    (uint64_t)ld.lpBaseOfDll + secHdr.VirtualAddress;
                patch_section_ranges.first = patchSectionAddress;
                patch_section_ranges.second =
                    patchSectionAddress + secHdr.Misc.VirtualSize;
                printf("%s section at: 0x%llx (size: 0x%x)\n",
                       patchSections.c_str(), patchSectionAddress,
          ...
          if (hasRVA && waitForModule &&
              lowerLoaded.find(lowerTarget) != std::wstring::npos) {
            moduleBase = (uint64_t)ld.lpBaseOfDll;
            uint64_t targetAddr = moduleBase + targetRVA;

            HANDLE hThread =
                OpenThread(THREAD_ALL_ACCESS, FALSE, dbgEvent.dwThreadId);
            if (hThread) {
          ...
          if (lowerLoaded.find(L"kernelbase.dll") != std::wstring::npos) {
            ...
            LOG(L"[+] kernelbase.dll loaded at 0x" << std::hex
                                                   << kernelBase_address);
          }
#endif
#if analyze_ENABLED
          ...
          if (waitForModule && !hasRVA &&
            ...
            auto modEntryRVA = GetEntryPointRVA(buffer);
            auto modTLSRVAs = GetTLSCallbackRVAs(buffer);
            valid_ranges.emplace_back(moduleBase,
                                      moduleBase + optionalHeader.SizeOfImage);
            HANDLE hThread =
                OpenThread(THREAD_ALL_ACCESS, FALSE, dbgEvent.dwThreadId);
            if (modEntryRVA)
              modTLSRVAs.push_back(modEntryRVA);
    ...
    case CREATE_THREAD_DEBUG_EVENT: {
      if (bpType != BreakpointType::ExecGuard) {
        ...
        if (hThread && GetThreadContext(hThread, &ctx)) {
          ...
          if (IsInEmulationRange(entryAddress)) {
#if analyze_ENABLED
            LOG_analyze(GREEN, "New thread created! EntryPoint: 0x"
                                   << std::hex << entryAddress);

#endif

            CPU cpu(hThread);
            cpu.ThreadState = ThreadState::Unknown;
            ...
            if (bpType == BreakpointType::Hardware)
              ...
              if (breakpoints.find(ctx.Rip) == breakpoints.end()) {
                if (SetBreakpoint(pi.hProcess, ctx.Rip, orig))
                  breakpoints[ctx.Rip] = {orig, 1};
              }
            }
          }
...
#if analyze_ENABLED
            LOG_analyze(GREEN,
                        "New thread created! RIP: 0x" << std::hex << ctx.Rip);
          ...
          if (ReadProcessMemory(pi.hProcess, (LPCVOID)pointer, &address,
                                sizeof(address), nullptr) &&
              IsInEmulationRange(address)) {
#if analyze_ENABLED
            LOG_analyze(GREEN,
                        "New thread created! RDX: 0x" << std::hex << address);
#endif
            CPU cpu(hThread);
            cpu.ThreadState = ThreadState::Unknown;
            cpuThreads.emplace(dbgEvent.dwThreadId, std::move(cpu));
    ...
    case CREATE_PROCESS_DEBUG_EVENT: {
...
#if AUTO_PATCH_HW
      if (patchModule.empty() && !patchSection.empty() &&
          patchSectionAddress == 0) {

        IMAGE_DOS_HEADER dosHdr{};
        ReadProcessMemory(pi.hProcess, (LPCVOID)baseAddress, &dosHdr,
                          sizeof(dosHdr), nullptr);

        IMAGE_NT_HEADERS64 ntHdr{};
        ReadProcessMemory(pi.hProcess, (LPCVOID)(baseAddress + dosHdr.e_lfanew),
        ...
        for (DWORD i = 0; i < numberOfSections; i++) {
          ...
          ReadProcessMemory(
              ...
              &secHdr, sizeof(secHdr), nullptr);
          sections.push_back(secHdr);
          if (strncmp((char *)secHdr.Name, (char *)patchSections.c_str(),
                      patchSection.size()) == 0) {
            patch_modules_ranges.first = baseAddress;
            patch_modules_ranges.second =
                baseAddress + ntHdr.OptionalHeader.SizeOfImage;
      ...
      valid_ranges.emplace_back(baseAddress,
                                baseAddress + optionalHeader.SizeOfImage);
      ...
      if (hasRVA && !waitForModule) {
        ...
        LOG(L"[+] Breakpoint set on main executable at RVA 0x%llX -> 0x%llX",
            targetRVA, targetAddr);
      }

      if (!waitForModule && !hasRVA) {
        if (entryRVA)
          tlsRVAs.push_back(entryRVA);
    ...
    case EXCEPTION_DEBUG_EVENT: {
      ...
      case EXCEPTION_BREAKPOINT:
        if (bpType == BreakpointType::Software && breakpoints.count(exAddr)) {
          ...
          for (auto *cpu : cpus) {
            cpu->ThreadState = ThreadState::Running;
          ...
          for (size_t i = 0; i < cpus.size(); ++i) {
            ...
            workers.emplace_back([cpu, &returnedAddrs, i]() {
              returnedAddrs[i] = cpu->StartEmulation();
          ...
          for (auto addr : returnedAddrs) {
            ...
            if (breakpoints.find(addr) == breakpoints.end()) {
              ...
              if (SetBreakpoint(pi.hProcess, addr, orig)) {
                breakpoints[addr] = {orig, 1};
                LOG(L"[+] Breakpoint set at new address: 0x" << std::hex
                                                             << addr);
              }
            } else {
          ...
          if (bp.remainingHits > 0) {
            ...
            LOG(L"[*] Breakpoint at 0x" << std::hex << exAddr
                                        << L" removed permanently");
          }

          if (hThreadTrigger)
            CloseHandle(hThreadTrigger);
        }
        break;
        ...
        if (bpType == BreakpointType::Software && breakpoints.count(exAddr)) {
          ...
          if (it == cpuThreads.end()) {
            ...
            if (hThread) {
              ...
              cpu.ThreadState = ThreadState::Unknown;
              cpuThreads.emplace(dbgEvent.dwThreadId, std::move(cpu));
              LOG(L"[+] Created new CPU object for missing thread: "
                  << dbgEvent.dwThreadId);
          ...
          if (it != cpuThreads.end()) {
            ...
            uint64_t addr = cpu.StartEmulation();
      ...
      case EXCEPTION_ACCESS_VIOLATION:
        if (bpType == BreakpointType::ExecGuard) {
          ...
          if (bpType == BreakpointType::Software) {
            std::cout << " (Writing INT3 on code integrity protected program "
                         "can cause this. Consider using hardware breakpoint "
                         "mode)";
          }
          std::cout << std::endl;
        }
      ...
      case EXCEPTION_ILLEGAL_INSTRUCTION:
        std::cout << "[!] Illegal instruction at 0x" << std::hex << exAddr
                  << std::endl;
        break;

      case 0xC0000409: // STATUS_STACK_BUFFER_OVERRUN
        std::cout << "[!] Stack buffer overrun detected at 0x" << std::hex
                  << exAddr << std::endl;
        break;

      case EXCEPTION_STACK_OVERFLOW:
        std::cout << "[!] Stack overflow at 0x" << std::hex << exAddr
                  << std::endl;
        break;

      case EXCEPTION_INT_DIVIDE_BY_ZERO:
        std::cout << "[!] Divide by zero at 0x" << std::hex << exAddr
                  << std::endl;
        break;

      case EXCEPTION_PRIV_INSTRUCTION:
        std::cout << "[!] Privileged instruction exception at 0x" << std::hex
                  << std::endl;
        break;

      case 0x406d1388: // DBG_PRINTEXCEPTION_C
        std::cout << "[i] Debug string output exception at 0x" << std::hex
                  << exAddr << std::endl;
        break;

      default:
        std::cout << "[!] Unhandled exception 0x" << std::hex << exceptionCode
                  << " at 0x" << exAddr << std::endl;
        break;
      }
    ...
    case EXIT_THREAD_DEBUG_EVENT:
      ...
      LOG("Thread " << dbgEvent.dwThreadId << " exit");
      break;
